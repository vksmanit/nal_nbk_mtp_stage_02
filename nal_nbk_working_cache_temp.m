...
%%%%%%%%%%%%%%we have to write a function to remove coloops %%%%%%%%%%%%
%
%
%unction removed_coloops_g1_of_NBK = remove_coloops(node,g1,end_node)
%
%   global edges_for_NBK;
%   global node_visited;
%   edgeId = g1{node};
...
%unction  dfs_to_get_fundamental_circuit(start_node,end_node)
%   global edges_for_NBK;
%   global node_visited;
%   global g1_of_tree_of_NBK;
%   global MyCktIds ;
...
%       elseif (node_visited(otherNode) == 1)
%           if (length(g1_of_tree_of_NBK{start_node}) == 1) %%%% remove coloops here
%               % how to write code here to combine the both the condition
%              %if (length(MyCktIds) == 1) 
%              %    MyCktIds = [];
%              %else
%              %    MyCktIds = MyCktIds(1:end-1);
%              %end
%              MyCktIds = recursive_call_to_remove_coloops(start_node,g1_of_tree_of_NBK);
%                   
%           end
%               continue;
%           if (length(MyCktIds) == 1) 
...
%nd
%
%unction MyCktIds =  recursive_call_to_remove_coloops(node, recursive_g1_of_tree_of_NBK);
%   %global recursive_g1_of_tree_of_NBK;
%    global g1_of_tree_of_NBK;
%   global MyCktIds;
%   global edges_for_NBK;
%
%   if (length(recursive_g1_of_tree_of_NBK{node}) ~= 1) 
%       return ;
%   end
%
%   if (length(recursive_g1_of_tree_of_NBK{node}) == 1) 
%
%       edgeId =  recursive_g1_of_tree_of_NBK{node};
%       edge = edges_for_NBK(edgeId, :);
%       MyCktIds = MyCktIds(1:end-1);
%       recursive_g1_of_tree_of_NBK{node} = [];
%       otherNode = edge(1,2);
%       
%       if (strcmp(otherNode,'gnd'))
...
%       end
%
%       recursive_g1_of_tree_of_NBK{otherNode} = recursive_g1_of_tree_of_NBK{otherNode}(~edgeId);
%       recursive_call_to_remove_coloops(otherNode, recursive_call_to_remove_coloops);
%   end
%   
%nd
%
